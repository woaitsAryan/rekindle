...
export const usageMiddleware = createMiddleware(
	async (c: Context<AuthenticatedEnv & BilledUsageEnv>, next) => {
		...
		if (!billedPlan) {
			...
			throw new HTTPException(404, {
				message: "Billed plan not found for the customer",
			});
		}
		...
		if (!matchedBilledUsage) {
			logger.error(
				"Unable to find a matching billed usage rule for customer",
				user.id,
			);
			throw new HTTPException(404, {
				message: "No matching billed usage found",
			});
		}
